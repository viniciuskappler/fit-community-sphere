    ...
    <section className="w-full px-4 md:px-8 lg:px-4 flex flex-col items-center bg-white py-[31px]">
      <div className="w-full md:w-11/12 lg:w-3/4 mx-auto">
        ...
        <div className="relative w-full h-[600px] rounded-xl overflow-hidden">
          ...
          <div className="absolute inset-0 bg-gradient-to-l from-white/30 via-transparent to-white/30 px-0 mx-0 py-0" />
          
          {/* Map pins - estilo Google Maps */}
          {locations.map((location, index) => (
            <div key={location.id} className="absolute transform -translate-x-1/2 -translate-y-full cursor-pointer transition-all duration-200 hover:scale-110 z-10" style={{
              left: `${location.x}%`,
              top: `${location.y}%`
      ...
      {hoveredLocation && (
        <div className="fixed z-50 bg-white rounded-lg shadow-xl border border-gray-200 overflow-hidden w-72 pointer-events-none transform -translate-x-1/2 -translate-y-full" style={{
